...
public class PowerFistRenderer extends ItemStackTileEntityRenderer {
    ...
    public void renderByItem/*render*/(ItemStack stack, ItemCameraTransforms.TransformType transformType, MatrixStack matrixStack, IRenderTypeBuffer buffer, int combinedLight, int combinedOverlay) {
        IVertexBuilder builder = buffer.getBuffer(RenderType.entityTranslucentCull(MPSConstants.POWER_FIST_TEXTURE));
        powerFist.renderToBuffer(matrixStack, builder, combinedLight, combinedOverlay, 1F, 1F, 1F, 1F);
...
//                            MPSPackets.CHANNEL_INSTANCE.sendToServer(new CosmeticInfoPacket(EquipmentSlotType.MAINHAND, NuminaConstants.TAG_RENDER, renderSpec));
//                        } else if (player.getOffhandItem().equals(stack)) {
//                            specNBTCap.setRenderTag(renderSpec, NuminaConstants.TAG_RENDER);
//                            MPSPackets.CHANNEL_INSTANCE.sendToServer(new CosmeticInfoPacket(EquipmentSlotType.OFFHAND, NuminaConstants.TAG_RENDER, renderSpec));
//                        }
//                    }
//                }
//
...
//
//
//                                    matrixStack.mulPose(transform.getRightRot());
//
//
//                                    matrixStack.scale(transform.getScale().x(), transform.getScale().y(), transform.getScale().z());
//                                    matrixStack.popPose();
//
////
