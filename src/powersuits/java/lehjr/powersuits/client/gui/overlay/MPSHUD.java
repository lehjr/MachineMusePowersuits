...
public class MPSHUD {
    ...
    public static void render(Player player, PoseStack poseStack, float top , float left) {
                    ...
                    module.getCapability(NuminaCapabilities.POWER_MODULE).ifPresent(pm->{
                        if (pm.isModuleOnline()) {
                            ...
                            if (ItemUtils.getRegistryName(module).equals(MPSRegistryNames.AUTO_FEEDER_MODULE)) {
                                ...
                                StringUtils.drawShadowedString(poseStack, num, 17, 4 +  top + (yOffsetString * index.get()));
                                // FIXME
                                NuminaRenderer.drawItemAt(poseStack, -1.0, top + (yOffsetIcon * index.get()), food, Color.WHITE);
                                index.addAndGet(1);
                                ...
                                if (pm.isModuleOnline()) {
                                    ...
                                    StringUtils.drawShadowedString(poseStack, hour + ampm, left + 17, 4 + top  + (yOffsetString * index.get()));
                                    NuminaRenderer.drawItemAt(poseStack, left -1.0, top + (yOffsetIcon * index.get()), clock, Color.WHITE);
                                    index.addAndGet(1);
                                }
                                // Generic modules
                            } else if (pm instanceof IHudModule) {
                                NuminaRenderer.drawItemAt(poseStack, left -1.0, top + (yOffsetIcon * index.get()), module, Color.WHITE);
                                index.addAndGet(1);
                            }
                        }
                    });
