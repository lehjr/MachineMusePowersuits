...
//        }
    ...
    public static int getItemEnergy(@Nonnull ItemStack itemStack) {
        return NuminaCapabilities.getCapability(itemStack, Capabilities.EnergyStorage.ITEM).filter(iEnergyStorage -> iEnergyStorage.getMaxEnergyStored() >= getMaxEnergyForComparison()).map(IEnergyStorage::getEnergyStored).orElse(0);
    }
    ...
    public static int getMaxItemEnergy(@Nonnull ItemStack itemStack) {
        return NuminaCapabilities.getCapability(itemStack, Capabilities.EnergyStorage.ITEM).filter(iEnergyStorage -> iEnergyStorage.getMaxEnergyStored() >= getMaxEnergyForComparison()).map(IEnergyStorage::getMaxEnergyStored).orElse(0);
    }

    public static int chargeItem(@Nonnull ItemStack itemStack, double chargeAmount, boolean simulate) {
        return NuminaCapabilities.getCapability(itemStack, Capabilities.EnergyStorage.ITEM).map(energyHandler -> energyHandler.receiveEnergy((int)chargeAmount, simulate)).orElse(0);
    }
