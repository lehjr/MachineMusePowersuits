...
public class ModularItemSelectionFrame extends AbstractGuiFrame<DrawableTile> {
    ...
    Optional<IModularItem> getModularItemCapability (EquipmentSlot type) {
        Optional<IModularItem> modularItemCap = NuminaCapabilities.getCapability(getStack(type), NuminaCapabilities.Inventory.MODULAR_ITEM);

        if(modularItemCap.isEmpty()) {
            modularItemCap = NuminaCapabilities.getCapability(getStack(type), NuminaCapabilities.Inventory.MODE_CHANGING_MODULAR_ITEM)
