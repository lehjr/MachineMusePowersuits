...
public class MPSRecipeTabToggleWidget extends RecipeTabToggleWidget {
    ...
    DrawableMuseRect tabRectangle = new DrawableMuseRect(0, 0, 0, 0, inactiveColor, Colour.LIGHT_BLUE.withAlpha(0.8F));;
    NonNullList<ItemStack> displayStacks;
    private final RecipeBookCategories category;


    public MPSRecipeTabToggleWidget(NonNullList<ItemStack> displayStacksIn) {
...
public class MPSRecipeTabToggleWidget extends RecipeTabToggleWidget {
    ...
    private void renderIcon(ItemRenderer renderer) {
        ...
        if (displayStacks.isEmpty()) {
            displayStacks.addAll(this.category.getIcons());
        }

        if (displayStacks.size() == 1) {
            renderer.renderItemAndEffectIntoGUI(displayStacks.get(0), this.x + 9 + offset, this.y + 5);
        } else if (displayStacks.size() == 2) {
            renderer.renderItemAndEffectIntoGUI(displayStacks.get(0), this.x + 3 + offset, this.y + 5);
            renderer.renderItemAndEffectIntoGUI(displayStacks.get(1), this.x + 14 + offset, this.y + 5);
        }
    }
}