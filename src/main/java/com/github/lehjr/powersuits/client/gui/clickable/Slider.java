...
public class Slider extends DrawableTile implements IClickable {
    ...
    public void update(double mouseX, double mouseY) {
        ...
        if (this.isEnabled() && this.isVisible() && dragging) {
            if (isHorizontal) {
                this.internalVal = MuseMathUtils.clampDouble((mouseX - centerx()) / (this.size - knobRect.finalWidth() * 0.5) + 0.5, 0.0, 1.0);
            } else {
                this.internalVal = MuseMathUtils.clampDouble((mouseY - centery()) / (this.size - knobRect.finalHeight() * 0.5) + 0.5, 0.0, 1.0);
            }
        } else {
            this.internalVal = MuseMathUtils.clampDouble(internalVal, 0.0, 1.0);
        }
    ...
    public void setValue(double value) {
        internalVal = MuseMathUtils.clampDouble(value, 0, 1);
    }

    public void setValueByX(double value) {
        if (isHorizontal) {
            this.internalVal = MuseMathUtils.clampDouble((value - centerx()) / this.width() + 0.5D, 0.0D, 1.0D);
        } else {
            internalVal = MuseMathUtils.clampDouble((value - centery()) / size + 0.5F, 0, 1);
        }
    }
        ...
        if (tickVal !=0) {
            for (double i = getMinVal() + tickVal; i < getMaxValue(); i += tickVal) {
                vals.add((this.isHorizontal ?
                        finalLeft() : finalTop())
                        + this.size  * (MuseMathUtils.clampDouble((i - getMinVal()) / (getMaxValue() - getMinVal()), 0.0, 1.0)));
            }
        }
        return vals;
    }
