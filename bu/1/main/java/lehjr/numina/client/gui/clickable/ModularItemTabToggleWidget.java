...
public class ModularItemTabToggleWidget extends Clickable {
    ...
    public ModularItemTabToggleWidget(EquipmentSlot type) {
        ...
        switch (type.getType()) {
            case HAND -> this.icon = NuminaCapabilities.getCapability(test, NuminaCapabilities.MODE_CHANGING_MODULAR_ITEM)
                    .map(modularItem -> test).orElse(ItemStack.EMPTY);
            case ARMOR -> this.icon = NuminaCapabilities.getCapability(test, NuminaCapabilities.MODULAR_ITEM)
                    .map(modularItem -> test).orElse(ItemStack.EMPTY);
        }
    }
