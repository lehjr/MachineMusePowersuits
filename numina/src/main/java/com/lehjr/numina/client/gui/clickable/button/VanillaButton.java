package com.lehjr.numina.client.gui.clickable.button;
...
public class VanillaButton extends Clickable {
    ...
    Component label;
    static final ResourceLocation BUTTON = ResourceLocation.fromNamespaceAndPath("minecraft", "widget/button");
    static final ResourceLocation BUTTON_DISABLED = ResourceLocation.fromNamespaceAndPath("minecraft","widget/button_disabled");
    static final ResourceLocation BUTTON_HIGHLIGHTED = ResourceLocation.fromNamespaceAndPath("minecraft","widget/button_highlighted");
...
////        RenderSystem.setShaderTexture(0, AbstractWidget.WIDGETS_LOCATION); // FIXME!!!: see vanilla's AbstractButton
...
//    }
    ...
    protected void renderButton(GuiGraphics gfx, int mouseX, int mouseY, float partialTick) {
        Minecraft minecraft = Minecraft.getInstance();
        ...
        if (this.isEnabled()) {
            resourceLocation = this.containsPoint(mouseX, mouseY) ? BUTTON_HIGHLIGHTED : BUTTON;
        } else {
            resourceLocation = BUTTON_DISABLED;
        }
        gfx.blitSprite(resourceLocation, (int)this.left(), (int)this.top(), (int)this.width(), (int)this.height());
        gfx.setColor(1.0F, 1.0F, 1.0F, 1.0F);
//        int i = getFGColor();
//        this.renderString(gfx, minecraft.font, i | Mth.ceil(this.alpha * 255.0F) << 24);
        int i = getFGColor();

        gfx.drawCenteredString(StringUtils.getFontRenderer(), this.getLabel(), (int) (this.left() + this.width() / 2), (int) (this.top() + (this.height() - 8) / 2), i | Mth.ceil(this.alpha * 255.0F) << 24);
    }
