...
import com.lehjr.numina.common.utils.*;
...
public class MPSMeterOverlay {
    ...
    public static void render(Player player, GuiGraphics poseStack, float partialTick, float scaledWidth, float scaledHeight) {
        ...
        NuminaCapabilities.getCapability(ItemUtils.getItemFromEntitySlot(player, EquipmentSlot.CHEST), Capabilities.FluidHandler.ITEM).ifPresent(fh -> {
        ...
        if (player.isUsingItem()) {
            NuminaCapabilities.getCapability(player.getItemInHand(player.getUsedItemHand()), NuminaCapabilities.Inventory.MODE_CHANGING_MODULAR_ITEM)
